...
void node_free(csq_node* node) {
    ...
    switch (node->type) {
        ...
        case NODE_CONST_DECL:
            if (node->data.var_decl.name) node_free(node->data.var_decl.name);
            if (node->data.var_decl.type_spec) node_free(node->data.var_decl.type_spec);
            if (node->data.var_decl.init) node_free(node->data.var_decl.init);
            break;
        case NODE_FUNCTION_DECL:
            if (node->data.function_decl.name) node_free(node->data.function_decl.name);
            node_list_free(&node->data.function_decl.params);
            if (node->data.function_decl.return_type) node_free(node->data.function_decl.return_type);
            if (node->data.function_decl.body) node_free(node->data.function_decl.body);
            node_list_free(&node->data.function_decl.generics);
            break;
        case NODE_PARAM:
            if (node->data.param.name) node_free(node->data.param.name);
            if (node->data.param.type_spec) node_free(node->data.param.type_spec);
            if (node->data.param.default_val) node_free(node->data.param.default_val);
            break;
        case NODE_STRUCT_DECL:
            if (node->data.struct_decl.name) node_free(node->data.struct_decl.name);
            ...
            break;
        case NODE_ENUM_DECL:
            if (node->data.enum_decl.name) node_free(node->data.enum_decl.name);
            node_list_free(&node->data.enum_decl.variants);
            break;
        case NODE_ENUM_VARIANT:
            if (node->data.enum_variant.name) node_free(node->data.enum_variant.name);
            node_list_free(&node->data.enum_variant.fields);
            break;
        case NODE_FIELD:
            if (node->data.field.name) node_free(node->data.field.name);
            if (node->data.field.type_spec) node_free(node->data.field.type_spec);
            break;
        case NODE_IF:
            if (node->data.if_stmt.condition) node_free(node->data.if_stmt.condition);
            if (node->data.if_stmt.then_branch) node_free(node->data.if_stmt.then_branch);
            if (node->data.if_stmt.else_branch) node_free(node->data.if_stmt.else_branch);
            break;
        case NODE_SWITCH:
            if (node->data.switch_stmt.expr) node_free(node->data.switch_stmt.expr);
            node_list_free(&node->data.switch_stmt.cases);
            if (node->data.switch_stmt.default_case) node_free(node->data.switch_stmt.default_case);
            break;
        case NODE_CASE:
            if (node->data.case_stmt.value) node_free(node->data.case_stmt.value);
            if (node->data.case_stmt.body) node_free(node->data.case_stmt.body);
            break;
        case NODE_WHILE:
            if (node->data.while_stmt.condition) node_free(node->data.while_stmt.condition);
            if (node->data.while_stmt.body) node_free(node->data.while_stmt.body);
            break;
        case NODE_FOR:
            if (node->data.for_stmt.var) node_free(node->data.for_stmt.var);
            if (node->data.for_stmt.iterable) node_free(node->data.for_stmt.iterable);
            if (node->data.for_stmt.body) node_free(node->data.for_stmt.body);
            break;
        case NODE_REPEAT:
            if (node->data.repeat_stmt.body) node_free(node->data.repeat_stmt.body);
            if (node->data.repeat_stmt.condition) node_free(node->data.repeat_stmt.condition);
            break;
        case NODE_RETURN:
            if (node->data.return_stmt.value) node_free(node->data.return_stmt.value);
            break;
        case NODE_THROW:
            if (node->data.throw_stmt.value) node_free(node->data.throw_stmt.value);
            break;
        case NODE_TRY:
            if (node->data.try_stmt.try_block) node_free(node->data.try_stmt.try_block);
            node_list_free(&node->data.try_stmt.catch_blocks);
            break;
        case NODE_CATCH:
            if (node->data.catch_block.var) node_free(node->data.catch_block.var);
            if (node->data.catch_block.type_spec) node_free(node->data.catch_block.type_spec);
            if (node->data.catch_block.block) node_free(node->data.catch_block.block);
            break;
        case NODE_DEFER:
            if (node->data.defer_stmt.stmt) node_free(node->data.defer_stmt.stmt);
            break;
        case NODE_BINARY_OP:
        ...
        case NODE_CALL:
            ...
            break;
        case NODE_INDEX:
            if (node->data.index.object) node_free(node->data.index.object);
            if (node->data.index.index) node_free(node->data.index.index);
            break;
        case NODE_ACCESS:
            if (node->data.access.object) node_free(node->data.access.object);
            if (node->data.access.member) node_free(node->data.access.member);
            break;
        ...
        case NODE_MAP_LITERAL:
            ...
            break;
        case NODE_MAP_ENTRY:
            if (node->data.map_entry.key) node_free(node->data.map_entry.key);
            if (node->data.map_entry.value) node_free(node->data.map_entry.value);
            break;
        case NODE_SPAWN:
            if (node->data.spawn.call_expr) node_free(node->data.spawn.call_expr);
            break;
        case NODE_IMPORT:
            free((void*)node->data.import.path);
            if (node->data.import.alias) node_free(node->data.import.alias);
            break;
        case NODE_CAST:
            if (node->data.cast.expr) node_free(node->data.cast.expr);
            if (node->data.cast.target_type) node_free(node->data.cast.target_type);
            break;
        case NODE_TYPE_SPEC:
            if (node->data.type_spec.base) node_free(node->data.type_spec.base);
            node_list_free(&node->data.type_spec.args);
            break;
        case NODE_GENERIC_INST:
            if (node->data.generic_inst.base) node_free(node->data.generic_inst.base);
            node_list_free(&node->data.generic_inst.type_args);
            break;
        case NODE_RANGE:
            if (node->data.range.start) node_free(node->data.range.start);
            if (node->data.range.end) node_free(node->data.range.end);
            break;
        case NODE_TUPLE:
...
void ast_context_free(ast_context* ctx) {
    ...
    if (ctx->root) node_free(ctx->root);
    if (ctx->symbols) sym_table_free(ctx->symbols);
...
const char* node_type_to_string(node_type type) {
    switch (type) {
        ...
        case NODE_LITERAL_TAG: return "literal_tag";
        case NODE_ARRAY_LITERAL: return "array_literal";
        case NODE_MAP_LITERAL: return "map_literal";
