...
void node_list_add(node_list *list, rvn_node *node) {
  ...
  if (list->count >= list->capacity) {
    ...
    rvn_node **new_items =
        realloc(list->items, new_capacity * sizeof(rvn_node *));
    if (!new_items)
      return;
...
rvn_node *ast_create_var_decl(rvn_node *name, rvn_node *type_spec,
  ...
  rvn_node *node = node_create(NODE_VAR_DECL, name ? name->line : 0,
                               name ? name->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_const_decl(rvn_node *name, rvn_node *type_spec,
                                rvn_node *init) {
  rvn_node *node = node_create(NODE_CONST_DECL, name ? name->line : 0,
                               name ? name->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_function_decl(rvn_node *name, node_list *params,
  ...
  rvn_node *node = node_create(NODE_FUNCTION_DECL, name ? name->line : 0,
                               name ? name->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_struct_decl(rvn_node *name, node_list *fields) {
  rvn_node *node = node_create(NODE_STRUCT_DECL, name ? name->line : 0,
                               name ? name->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_enum_decl(rvn_node *name, node_list *variants) {
  rvn_node *node = node_create(NODE_ENUM_DECL, name ? name->line : 0,
                               name ? name->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_enum_variant(rvn_node *name, node_list *fields,
                                  int value) {
  rvn_node *node = node_create(NODE_ENUM_VARIANT, name ? name->line : 0,
                               name ? name->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_if(rvn_node *condition, rvn_node *then_branch,
  ...
  rvn_node *node = node_create(NODE_IF, condition ? condition->line : 0,
...
rvn_node *ast_create_case(rvn_node *value, rvn_node *body) {
  rvn_node *node = node_create(NODE_CASE, value ? value->line : 0,
...
rvn_node *ast_create_while(rvn_node *condition, rvn_node *body) {
  rvn_node *node = node_create(NODE_WHILE, condition ? condition->line : 0,
                               condition ? condition->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_return(rvn_node *value) {
  rvn_node *node = node_create(NODE_RETURN, value ? value->line : 0,
                               value ? value->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_throw(rvn_node *value) {
  rvn_node *node = node_create(NODE_THROW, value ? value->line : 0,
                               value ? value->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_try(rvn_node *try_block, node_list *catches) {
  rvn_node *node = node_create(NODE_TRY, try_block ? try_block->line : 0,
...
rvn_node *ast_create_catch(rvn_node *var, rvn_node *type_spec,
                           rvn_node *block) {
  rvn_node *node =
...
rvn_node *ast_create_break(void) { return node_create(NODE_BREAK, 0, 0); }

rvn_node *ast_create_continue(void) { return node_create(NODE_CONTINUE, 0, 0); }

rvn_node *ast_create_binary(binary_op op, rvn_node *left, rvn_node *right) {
  rvn_node *node = node_create(NODE_BINARY_OP, left ? left->line : 0,
                               left ? left->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_unary(unary_op op, rvn_node *operand) {
  rvn_node *node = node_create(NODE_UNARY_OP, operand ? operand->line : 0,
...
rvn_node *ast_create_call(rvn_node *callee, node_list *args) {
  rvn_node *node = node_create(NODE_CALL, callee ? callee->line : 0,
...
rvn_node *ast_create_index(rvn_node *object, rvn_node *index) {
  rvn_node *node = node_create(NODE_INDEX, object ? object->line : 0,
...
rvn_node *ast_create_access(rvn_node *object, rvn_node *member, bool is_arrow) {
  rvn_node *node = node_create(NODE_ACCESS, object ? object->line : 0,
                               object ? object->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_spawn(rvn_node *call_expr) {
  rvn_node *node = node_create(NODE_SPAWN, call_expr ? call_expr->line : 0,
...
rvn_node *ast_create_self(void) { return node_create(NODE_SELF, 0, 0); }
...
rvn_node *ast_create_type_spec(rvn_node *base, node_list *args) {
  rvn_node *node = node_create(NODE_TYPE_SPEC, base ? base->line : 0,
                               base ? base->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_generic_inst(rvn_node *base, node_list *type_args) {
  rvn_node *node = node_create(NODE_GENERIC_INST, base ? base->line : 0,
                               base ? base->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_range(rvn_node *start, rvn_node *end, bool inclusive) {
  rvn_node *node = node_create(NODE_RANGE, start ? start->line : 0,
...
rvn_node *ast_create_expr_stmt(rvn_node *expr) {
  rvn_node *node = node_create(NODE_EXPR_STMT, expr ? expr->line : 0,
                               expr ? expr->column : 0);
  if (!node)
    return NULL;
...
rvn_node *ast_create_empty(void) { return node_create(NODE_EMPTY, 0, 0); }
