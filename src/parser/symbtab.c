 ...
 * @details Initializes the global scope and sets up for nested scope management.
...
void sym_leave_scope(sym_table* table) {
    if (!table || !table->current || !table->current->parent) return;
...
struct csq_symbol* sym_lookup_recursive(sym_scope* scope, const char* name, size_t len) {
    while (scope) {
        unsigned int bucket_idx = sym_hash(name, len);
...
bool sym_is_in_loop(sym_table* table) {
    sym_scope* scope = table->current;
    while (scope) {
        if (scope->is_loop) return true;
        if (scope->is_function) return false;
        scope = scope->parent;
    }
    return false;
}
...
bool sym_is_in_function(sym_table* table) {
    sym_scope* scope = table->current;
    while (scope) {
        if (scope->is_function) return true;
        scope = scope->parent;
    }
    return false;
}
...
void sym_check_unused(sym_table* table) {
    ...
    while (scope) {
        for (int i = 0; i < SYM_HASH_SIZE; i++) {
            ...
            struct csq_symbol* sym = bucket->symbols;
            while (sym) {
                if (!sym->is_used && sym->kind == SYM_VARIABLE && !sym_is_global(table, sym)) {
                }
                sym = sym->next_in_scope;
            }
        }
