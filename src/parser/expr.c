...
csq_node* expr_parse_primary(csq_parser* parser) {
    ...
    if (parser_match(parser, TOKEN_NUMBER)) {
        ...
        if (end < token.start + token.length && (*end == '.' || *end == 'e' || *end == 'E')) {
    ...
    if (parser_match(parser, TOKEN_KEYWORD_SELF)) {
        return node_create(NODE_SELF, parser->previous.line, parser->previous.column);
    }
    ...
    if (parser_match(parser, TOKEN_OPEN_BRACKET)) {
        ...
        parser_consume(parser, TOKEN_CLOSE_BRACKET, "Expected ']' after array elements");
        return array;
    }
...
csq_node* expr_parse_multiplicative(csq_parser* parser) {
    ...
    for (;;) {
        binary_op op;
        if (parser_match(parser, TOKEN_STAR)) op = BINOP_MUL;
        else if (parser_match(parser, TOKEN_SLASH)) op = BINOP_DIV;
        else if (parser_match(parser, TOKEN_PERCENT)) op = BINOP_MOD;
        else break;
...
csq_node* expr_parse_additive(csq_parser* parser) {
    ...
    for (;;) {
        binary_op op;
        if (parser_match(parser, TOKEN_PLUS)) op = BINOP_ADD;
        else if (parser_match(parser, TOKEN_MINUS)) op = BINOP_SUB;
        else break;
