...
#if RVN_DEBUG_MODE
    fprintf(stderr, "%s %s%-5s%s %s[%s:%d]%s ",
            time_buf, level_colors[level], level_strings[level], reset_color,
            dim_color, file, line, reset_color);
#else
    (void)file;
    (void)line;
    (void)dim_color;
    fprintf(stderr, "%s%-5s%s ",
            level_colors[level], level_strings[level], reset_color);
#endif
