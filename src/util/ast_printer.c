 ...
 * @details Provides functions to print AST structures in a readable hierarchical
 * format for debugging and development.
 */
...
static void print_node_list(const node_list* list, FILE* out, int indent, const char* label);
...
void ast_print_node(const csq_node* node, FILE* out, int indent) {
    ...
    switch (node->type) {
        ...
        case NODE_FUNCTION_DECL:
            ...
            print_node_list(&node->data.function_decl.params, out, child_indent, "params");
            if (node->data.function_decl.return_type) {
                print_indent(out, child_indent);
                fprintf(out, "ret:\n");
                ast_print_node(node->data.function_decl.return_type, out, child_indent + 1);
            }
            print_indent(out, child_indent);
            fprintf(out, "body:\n");
        ...
        case NODE_IDENTIFIER:
            print_indent(out, child_indent);
            fprintf(out, "name: %.*s\n", (int)node->data.identifier.len, node->data.identifier.name);
            break;
        ...
        case NODE_LITERAL_STRING:
            print_indent(out, child_indent);
            fprintf(out, "value: \"%.*s\"\n", (int)node->data.literal_string.len, node->data.literal_string.value);
            break;
        ...
        case NODE_LITERAL_TAG:
            print_indent(out, child_indent);
            fprintf(out, "name: #%.*s\n", (int)node->data.literal_tag.len, node->data.literal_tag.name);
            break;
            
        case NODE_ARRAY_LITERAL:
            print_node_list(&node->data.array_literal.elements, out, child_indent, "elems");
            break;
...
void ast_print_type(const csq_type* type, FILE* out) {
    ...
    switch (type->kind) {
        ...
        case TYPE_F32: fprintf(out, "f32"); break;
        case TYPE_F64: fprintf(out, "f64"); break;

        case TYPE_STRING: fprintf(out, "string"); break;
        case TYPE_TAG: fprintf(out, "tag"); break;
        case TYPE_POINTER:
            fprintf(out, "*");
...
void ast_print_symbol(const struct csq_symbol* sym, FILE* out) {
    ...
    const char* kind_str = "?";
    switch (sym->kind) {
        case SYM_VARIABLE: kind_str = "var"; break;
        case SYM_CONSTANT: kind_str = "const"; break;
        case SYM_FUNCTION: kind_str = "fn"; break;
        case SYM_PARAMETER: kind_str = "param"; break;
        case SYM_STRUCT: kind_str = "struct"; break;
        case SYM_ENUM: kind_str = "enum"; break;
        case SYM_FIELD: kind_str = "field"; break;
        case SYM_TYPE_ALIAS: kind_str = "type"; break;
        default: break;
    }
