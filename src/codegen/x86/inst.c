...
void inst_mov_reg_mem(emit_ctx* ctx, x86_reg dst, x86_reg base, int offset) {
    if (offset == 0) emit_instruction(ctx, "movl", "(%s), %s", reg32_name(base), reg32_name(dst));
    else emit_instruction(ctx, "movl", "%d(%s), %s", offset, reg32_name(base), reg32_name(dst));
    
}

void inst_mov_mem_reg(emit_ctx* ctx, x86_reg base, int offset, x86_reg src) {
    if (offset == 0)  emit_instruction(ctx, "movl", "%s, (%s)", reg32_name(src), reg32_name(base));
    else emit_instruction(ctx, "movl", "%s, %d(%s)", reg32_name(src), offset, reg32_name(base));
}

void inst_mov_reg_mem_indexed(emit_ctx* ctx, x86_reg dst, x86_reg base, x86_reg index, int scale, int offset) {
    emit_instruction(ctx, "movl", "%d(%s,%s,%d), %s", offset, reg32_name(base), reg32_name(index), scale, reg32_name(dst));
}
...
void inst_ret(emit_ctx* ctx) {
    emit_instruction(ctx, "ret", NULL);
}

void inst_lea(emit_ctx* ctx, x86_reg dst, x86_reg base, int offset) {
    if (offset == 0) {
        emit_instruction(ctx, "leal", "(%s), %s", reg32_name(base), reg32_name(dst));
    } else {
        emit_instruction(ctx, "leal", "%d(%s), %s", offset, reg32_name(base), reg32_name(dst));
    }
}
...
void inst_nop(emit_ctx* ctx) {
    emit_instruction(ctx, "nop", NULL);
}

void inst_cdq(emit_ctx* ctx) {
    emit_instruction(ctx, "cdq", NULL);
}
